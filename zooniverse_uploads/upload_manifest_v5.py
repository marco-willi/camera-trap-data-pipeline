...
def create_subjects_from_captures(
    ...
    for capture_id in capture_ids_to_upload:
        ...
        if capture_id in batch_data['capture_ids']:
            logger.debug("capture_id {} alreday uploaded".format(capture_id))
            continue
...
if __name__ == "__main__":
    ...
    # generate id slices for each batch
    n_upload_batches = math.ceil(n_tot / upload_batch_size)
    upload_slices = slice_generator(n_tot, n_upload_batches)
    ...
    for batch_no, (start_i, end_i) in enumerate(upload_slices):
        ...
        # select data of the current batch
        capture_ids_batch = capture_ids_all[start_i: end_i]

        batch_data = batch_data_storage()

        try:
            retry(create_subjects_from_captures,
