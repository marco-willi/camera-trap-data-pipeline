...
for season_site_roll_key, site_roll_data in site_roll_inventory.items():
    ...
    datetimes = [datetime.strptime(x,
        flags['time_formats']['output_datetime_format']) for x in times]
    times_seconds = [(x-datetime(1970,1,1)).total_seconds() for x in datetimes]
    ...
    delta_times_days_ordered = ['{:.2f}'.format((x / (60*60*24)))
    ...
    # loop over all deltas (starting with the delta between first and second)
...
for image_path_original, image_capture_data in image_to_capture.items():
    ...
    # perform new checks
    time_checks = {'image_check__{}'.format(x): 0
